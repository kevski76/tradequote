    ...
    <section class="grid grid-cols-1 gap-6 xl:grid-cols-12">
        <div class="space-y-6 xl:col-span-7">
            ...
            <article class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-zinc-100 dark:bg-zinc-900 dark:ring-zinc-800">
                ...
                <div class="mt-4" x-data="{ includeGate: {{ ($gateWidth > 0 || isset($itemInputs['gate'])) ? 'true' : 'false' }} }">
                    <button
                        type="button"
        ...
        <div class="space-y-6 xl:col-span-5">
            ...
            <section class="grid grid-cols-1 gap-3 sm:grid-cols-3 xl:grid-cols-1">
                ...
                <button
                    type="button"
                    x-on:click="$wire.saveQuoteWithInputs($refs.length.value, $refs.gateWidth ? $refs.gateWidth.value : null)"
                    wire:loading.attr="disabled"
                    wire:target="saveQuoteWithInputs"
                ...
                >
                    <svg wire:loading wire:target="saveQuoteWithInputs" class="h-4 w-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <span wire:loading.remove wire:target="saveQuoteWithInputs">Update Quote</span>
                    <span wire:loading wire:target="saveQuoteWithInputs">Updating...</span>
                </button>

                @if ($status === \App\Models\Quotes::STATUS_DRAFT)
                    {{-- Send via WhatsApp --}}
                    <button
                        type="button"
                ...
                @if ($status === \App\Models\Quotes::STATUS_WORK_COMPLETE)
                    <button
                        type="button"
                        wire:click="prepareCompletionWhatsApp"
                        wire:loading.attr="disabled"
                        wire:target="prepareCompletionWhatsApp"
                        wire:loading.class="opacity-75 cursor-not-allowed"
                        class="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700 disabled:opacity-60"
                    >
                        <svg wire:loading.remove wire:target="prepareCompletionWhatsApp" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/>
                            <path d="M12 0C5.373 0 0 5.373 0 12c0 2.127.558 4.122 1.531 5.855L0 24l6.293-1.508A11.955 11.955 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.885 0-3.65-.502-5.18-1.381l-.371-.221-3.838.92.96-3.736-.242-.385A9.956 9.956 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/>
                        </svg>
                        <svg wire:loading wire:target="prepareCompletionWhatsApp" class="h-4 w-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                        <span wire:loading.remove wire:target="prepareCompletionWhatsApp">Send Completion WhatsApp</span>
                        <span wire:loading wire:target="prepareCompletionWhatsApp">Preparing...</span>
                    </button>
                @endif
